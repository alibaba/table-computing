...
import static com.alibaba.sdb.spi.StandardErrorCode.NOT_SUPPORTED;
import static com.alibaba.sdb.spi.block.AbstractMapBlock.HASH_MULTIPLIER;
import static com.alibaba.sdb.spi.block.MapBlockBuilder.computePosition;
        ...
        extends AbstractSingleMapBlock
{
    ...
    private static RuntimeException handleThrowable(Throwable throwable)
    {
        ...
        if (throwable instanceof SdbException) {
            throw (SdbException) throwable;
        }
        throw new SdbException(GENERIC_INTERNAL_ERROR, throwable);
    }

    private static void checkNotIndeterminate(Boolean equalsResult)
    {
        if (equalsResult == null) {
            throw new SdbException(NOT_SUPPORTED, "map key cannot be null or contain nulls");
        }
    }
}
