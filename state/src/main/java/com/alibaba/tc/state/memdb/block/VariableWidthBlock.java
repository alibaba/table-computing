        ...
        extends AbstractVariableWidthBlock
{
    ...
    VariableWidthBlock(int arrayOffset, int positionCount, Slice slice, int[] offsets, boolean[] valueIsNull)
    {
        ...
        sizeInBytes = offsets[arrayOffset + positionCount] - offsets[arrayOffset] + ((Integer.BYTES + Byte.BYTES) * (long) positionCount) + Integer.BYTES;
        retainedSizeInBytes = INSTANCE_SIZE + BlockUtil.sizeOf(slice) + sizeOf(valueIsNull) + sizeOf(offsets);
    }
    ...
    {
        return offsets[arrayOffset + position + length] - offsets[arrayOffset + position] + ((Integer.BYTES + Byte.BYTES) * (long) length) + Integer.BYTES;
    }
    ...
    @Override
    public Block compactRegionToOffheap(int positionOffset, int length)
    {
        return new VariableWidthBlockOffheap(positionOffset, length, slice, offsets, valueIsNull);
    }
